...
#define CONF 5             // confidence treshold
#define VELOCITY  0.5           // 1 m/s
...
void generate_pursuit_vector(int x, int y){
  angley = 0.5 * (1 * (HALFSIZE - x) * RANGEX/HALFSIZE);
  anglex = 0.5 * (-1 * (HALFWIDE - y) * RANGEY/HALFWIDE);
...
//Initialize the ros talkers and listeners
  ...
  ros::Subscriber sub = sub_target.subscribe("detection", 1, target_Callback);
...
//Loop every 0.2 seconds and read/write the messages
  while (ros::ok()){
     ...
     if(target_stall==1 && mode==0){
       msg.flag = 1;
	     msg.speed = 0;
       stall_countdown--;
       std::cout << "Countdown "<<stall_countdown<<"\n";
      }
    else
      msg.speed = magnitude;
