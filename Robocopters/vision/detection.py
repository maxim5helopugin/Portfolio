...
class Detector:
                ...
                if len(candidate_centers) > 0:
                    ...
                    if max_i >= 0:
                        ...
                        if self.lk_track and self.tracker.tracking < 1 and max_score > self.LOCK_THRESHOLD:
                            ...
                            pt1 = np.add(pt1, 2)
                            pt2 = np.add(pt2, -2)
                            region_pts = inner_points(pt1, pt2, np.array(candidate_centers))
                            if len(region_pts) > 2:
    ...
    def detection_offset(self, img, bounding_box, confidence=0, show=True):
        ...
        x = int((center_x / self.w) * 100)  # Convert coordinates to percentage of view window size
        ...
        if show:
            ...
            img = cv2.putText(img, text, (pt1[0], pt1[1] - 3), cv2.FONT_HERSHEY_PLAIN, 1.3, (255, 255, 255))    # label
            cv2.arrowedLine(img, (self.origin_x, self.origin_y), (center_x, center_y), color=(255, 255, 255),   # arrow
                            thickness=1)  # offset
        return img, x, y
...
def region_sum(mat, pt, width):
    """Sum of data values in 2D array within window surrounding point."""
    x, y = pt
    h, w = mat.shape[:2]
...
def midpoint(a, b):
    """Return coordinates of midpoint of two 2D points."""
...
def matching_keypoints(matches, kp):
    """Keypoints corresponding to match objects."""
